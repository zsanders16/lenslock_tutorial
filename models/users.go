...
import (
	...
	// required for postgres connection
...
// User contains all necessary info stored in DB for a user
type User struct {
	gorm.Model
	Name         string
...
// userGorm represents our database nteraction layer
...
// UserService interface implements the methods fo the userService struct
type UserService interface {
	// Authenticate verifies the provided email and password are correct.
	// Returns either the user or error
	Authenticate(email, password string) (*User, error)
	UserDB
}

// NewUserService instatiates a userGorm connection object
...
func (uv *userValidator) rememberMinBytes(user *User) error {
	...
	if n < 32 {
		return ErrRememberTokenIsTooShort
	}
	return nil
}
